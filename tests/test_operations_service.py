...
class OperationsServiceTests(unittest.TestCase):
    ...
    def test_request_router_adds_workspace_for_unsupported_skipped_phrase(self):
        service=OperationsService()
        item=service.create_work_item("Skipped", "Show unsupported skipped files in the workspace catalog")
        with patch.object(service, "local_plan_explanation", return_value="Safe workspace plan"):
            plan=service.plan_for_request(item["id"])
        self.assertIn("workspace.status", plan["actions"]); service.close()
    ...
    def test_request_router_adds_workspace_for_truncated_listing_phrase(self):
        service=OperationsService()
        item=service.create_work_item("Truncated", "Check truncated listing and listing truncated status")
        with patch.object(service, "local_plan_explanation", return_value="Safe workspace plan"):
            plan=service.plan_for_request(item["id"])
        self.assertIn("workspace.status", plan["actions"]); service.close()
    ...
    def test_request_router_adds_workspace_for_listing_limit_phrase(self):
        service=OperationsService()
        item=service.create_work_item("Limit", "Show listing limit and result limit for workspace inventory")
        with patch.object(service, "local_plan_explanation", return_value="Safe workspace plan"):
            plan=service.plan_for_request(item["id"])
        self.assertIn("workspace.status", plan["actions"]); service.close()
    ...
    def test_workspace_snapshot_includes_listing_limit(self):
        service = OperationsService()
        snap = service.workspace_snapshot()
        self.assertEqual(snap["listing_limit"], 100)
        service.close()
    ...
    def test_workspace_inventory_is_read_only_and_router_uses_allowlisted_action(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("Skipped 1 unsupported file", status["message"])
