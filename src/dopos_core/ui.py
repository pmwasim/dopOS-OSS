  ...
  <div class="shell">
    ...
    <main class="frame">
      ...
      <aside class="rail">
        ...
        <div class="side-section"><div class="side-head"><h3>Diary</h3><a class="side-link" href="/journal.md" download="dopos-journal.md" aria-label="Export Diary as Markdown">Export Markdown</a></div><div id="diary" class="empty">No activity yet. Export still downloads the current journal projection.</div></div>
      </aside>
    </main>
  </div>
<script>
...
async function loadTools() { try { const tools = await api('/tools/status'); for (const [name, value] of Object.entries(tools)) { const element = document.querySelector(`#tool-${name}`); if (!element) continue; const available = value.available && value.ok !== false; element.classList.toggle('status-off', !available); let tip = available ? 'Available locally' : (value.error || value.reason || 'Unavailable locally'); if (name === 'ci' && available && Array.isArray(value.runs) && value.runs.length) { const latest = value.runs[0]; tip = `${latest.conclusion || latest.status || 'checked'} · ${latest.workflowName || 'workflow'}`; } element.title = tip; } } catch { document.querySelectorAll('.tools span').forEach(element => element.classList.add('status-off')); } }
...
async function loadToday() { try { const state = await api('/today'); const lines = []; if (state.needs_decision.length) lines.push(`<div class="today-line"><button class="today-action" onclick="openWork(${state.needs_decision[0].work_item_id})">${state.needs_decision.length} decision${state.needs_decision.length === 1 ? '' : 's'} waiting</button><small>approval</small></div>`); if (state.in_motion.length) lines.push(`<div class="today-line"><button class="today-action" onclick="openWork(${state.in_motion[0].work_item_id})">${state.in_motion.length} approved plan${state.in_motion.length === 1 ? '' : 's'}</button><small>ready to run</small></div>`); if (!lines.length) lines.push('<div class="today-line"><strong>Nothing waiting</strong><small>new work can be planned</small></div>'); lines.push(`<div class="today-line"><strong>Recovery</strong><small>${state.recovery.audit_chain_valid ? 'audit verified' : 'needs review'} · ${state.recovery.backup_count} backup${state.recovery.backup_count === 1 ? '' : 's'} · ${state.recovery.retention && state.recovery.retention.configured ? 'retention on' : 'retention unset'}</small></div>`); const workspace = state.workspace || {}; lines.push(`<div class="today-line"><strong>Workspace</strong><small>${workspace.configured === false ? 'documents folder not configured' : `${workspace.document_count || 0} document${(workspace.document_count || 0) === 1 ? '' : 's'} · ${workspace.folder_count || 0} folder${(workspace.folder_count || 0) === 1 ? '' : 's'}`}</small></div>`); const safety = state.safety || {}; lines.push(`<div class="today-line"><strong>Safety</strong><small>${safety.execution_paused ? 'execution paused' : 'execution ready'} · kill switch ${escape(safety.kill_switch || 'unknown')}</small></div>`); const automation = state.automation || {}; lines.push(`<div class="today-line"><strong>Automation</strong><small>${automation.latest_result ? `${escape(automation.latest_result)} · ${escape(automation.latest_title || 'cycle')}` : (automation.configured === false ? 'loop evidence not configured' : 'no cycle recorded yet')}</small></div>`); const queue = state.queue || {}; lines.push(`<div class="today-line"><strong>Queue</strong><small>${queue.configured === false ? 'inbox not configured' : (queue.next_title ? `next · ${escape(queue.next_title)}${queue.count ? ` · ${queue.count} item${queue.count === 1 ? '' : 's'}` : ''}` : 'no queued work')}</small></div>`); today.innerHTML = lines.join(''); } catch { today.innerHTML = '<div class="empty">Today is unavailable.</div>'; } }
...
function formatResults(results) { return results.map(entry => { const result = entry.result; if (entry.action === 'status.summary') return `<div><strong>Host records</strong><p>${result.work_items} work items · ${result.plans} plans · ${result.audit_events} audit events</p></div>`; if (entry.action === 'docker.status') return `<div><strong>Docker status</strong><pre>${escape(result.containers || result.error || result.reason || 'No Docker details returned.')}</pre></div>`; if (entry.action === 'ci.status') return `<div><strong>Recent CI runs</strong>${result.ok ? (result.runs || []).map(run => `<div class="workspace-result">${escape(run.workflowName || 'Workflow')}<small>${escape(run.conclusion || run.status)} · ${escape(run.headSha.slice(0, 8))}</small></div>`).join('') || '<p>No CI runs recorded.</p>' : `<p>${escape(result.error || result.reason || 'CI status unavailable.')}</p>`}</div>`; if (entry.action === 'quality.status') return `<div><strong>Local quality checks</strong><p>${result.ok ? 'All fixed checks passed.' : 'One or more checks need attention.'}</p><div class="actions">${(result.checks || []).map(check => `<span>${escape(check.name)} · ${check.passed ? 'passed' : 'failed'}</span>`).join('')}</div></div>`; if (entry.action === 'workspace.status') return `<div><strong>Workspace inventory</strong><p>${result.count} local document${result.count === 1 ? '' : 's'} · ${result.folder_count || 0} folder${(result.folder_count || 0) === 1 ? '' : 's'}. Names and paths only.</p>${(result.folders || []).map(folder => `<div class="workspace-result">${escape(folder.path)}<small>folder</small></div>`).join('')}${(result.documents || []).map(document => `<div class="workspace-result">${escape(document.path)}<small>${escape(document.extension)} · ${document.size} bytes</small></div>`).join('')}</div>`; if (entry.action === 'workspace.snapshot') return `<div><strong>Workspace snapshot</strong><p>${result.document_count} local document${result.document_count === 1 ? '' : 's'} · ${result.folder_count || 0} folder${(result.folder_count || 0) === 1 ? '' : 's'} tracked in this metadata-only revision.</p><div class="actions"><span>${escape(result.catalog_revision || 'no catalog yet')}</span></div></div>`; if (entry.action === 'loop.status') return `<div><strong>Autonomous loop</strong><p>${result.count} recent cycle${result.count === 1 ? '' : 's'} summarized. Evidence stays local.</p>${(result.cycles || []).map(cycle => `<div class="workspace-result">${escape(cycle.title || cycle.id)}<small>${escape(cycle.result)} · ${escape(cycle.started_at || '')}</small></div>`).join('') || '<p>No cycle evidence recorded yet.</p>'}</div>`; if (entry.action === 'queue.status') return `<div><strong>Autonomous queue</strong><p>${result.count ? `${result.count} queued work item${result.count === 1 ? '' : 's'}. Titles only.` : 'No queued Markdown work items. Titles only when present.'}</p>${(result.items || []).map(item => `<div class="workspace-result">${escape(item.title)}<small>${escape(item.path)}</small></div>`).join('') || '<p>No queued work.</p>'}</div>`; if (entry.action === 'backup.create') return `<div><strong>Local backup created</strong><p>${escape(result.path)} · audit chain ${result.audit_chain_valid ? 'verified' : 'needs review'}</p></div>`; if (entry.action === 'backup.verify') return `<div><strong>Recovery backups</strong><p>${escape(result.message)}</p><p>${escape((result.retention && result.retention.message) || 'Retention unset.')}</p><div class="actions">${(result.backups || []).map(check => `<span>${escape(check.name)} · ${check.ok ? 'verified' : 'needs review'}</span>`).join('')}</div></div>`; if (entry.action === 'backup.retention') return `<div><strong>Backup retention</strong><p>${escape(result.message)}</p><div class="actions"><span>configured · ${result.configured ? 'yes' : 'no'}</span><span>prune · ${result.prune_enabled ? 'enabled' : 'disabled'}</span></div></div>`; if (entry.action === 'health.status') return `<div><strong>Runtime health</strong><p>${escape(result.status || 'unknown')} · audit ${result.audit_chain_valid ? 'verified' : 'needs review'} · ${result.execution_paused ? 'execution paused' : 'execution ready'}</p><div class="actions"><span>backups · ${result.backup_count ?? 0}</span><span>documents · ${(result.workspace && result.workspace.document_count) || 0}</span><span>retention · ${result.backup_retention && result.backup_retention.configured ? 'on' : 'unset'}</span></div></div>`; if (entry.action === 'tools.status') { const names = Object.keys(result || {}); return `<div><strong>Local tools</strong><p>${names.length ? 'Read-only availability for the control-room header tools.' : 'No local tools reported.'}</p><div class="actions">${names.map(name => { const value = result[name] || {}; const available = value.available && value.ok !== false; return `<span>${escape(name)} · ${available ? 'available' : 'unavailable'}</span>`; }).join('')}</div></div>`; } if (entry.action === 'control.status') return `<div><strong>Safety control</strong><p>Kill switch is ${escape(result.kill_switch || 'unknown')}. This check is read-only and does not change execution.</p></div>`; if (entry.action === 'diary.preview') return '<div><strong>Diary updated</strong><p>The approved run and its evidence have been recorded.</p></div>'; return `<div><strong>${escape(entry.action)}</strong><p>${escape(JSON.stringify(result))}</p></div>`; }).join(''); }
...
async function searchWorkspace(event) { event.preventDefault(); const query = document.querySelector('#workspace-query').value.trim(); const results = document.querySelector('#workspace-results'); results.innerHTML = '<div class="empty">Searching local names…</div>'; try { const state = await api(`/workspace?query=${encodeURIComponent(query)}`); const folders = (state.folders || []).map(folder => `<div class="workspace-result">${escape(folder.path)}<small>folder</small></div>`).join(''); const documents = state.documents.map(document => `<div class="workspace-result">${escape(document.path)}<small>${escape(document.extension)} · ${document.size} bytes</small></div>`).join(''); const prefix = (state.count || state.folder_count) ? '' : `<div class="empty">${state.configured ? (query ? 'No matching local documents.' : 'Workspace is ready and empty.') : 'Local workspace directory has not been created yet.'}</div>`; results.innerHTML = prefix + folders + documents; } catch (error) { results.innerHTML = `<div class="empty">${escape(error.message)}</div>`; } }
...
async function loadDiary() { try { const events = await api('/journal'); diary.innerHTML = events.slice(-6).reverse().map(event => `<div class="event">${escape(event.summary)}<time>${new Date(event.created_at).toLocaleString()}${event.detail ? ` · ${escape(event.detail)}` : ''}</time></div>`).join('') || '<div class="empty">No activity yet. Export still downloads the current journal projection.</div>'; } catch {} }
