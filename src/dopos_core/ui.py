...
async function openWork(id) { try { const item = await api(`/work-items/${id}`); const plan = item.plan; if (!plan) { thread.innerHTML = `<div class="plan"><h3>${escape(item.title)}</h3><p>${escape(item.request)}</p><p>No plan has been created yet.</p></div>`; return; } if (plan.state === 'awaiting_approval') { renderPlan(plan, 'Saved plan'); return; } current = plan; const outcome = plan.state === 'completed' ? `<h3>Completed</h3><p>Recorded result from the approved run.</p><div class="results">${formatResults(plan.results || [])}</div>` : `<p>This plan is ${escape(plan.state)}.</p>`; thread.innerHTML = `<div class="message user">${escape(item.request)}</div><div class="agent"><div class="orb"></div><div><span class="label">dopOS</span><p class="explain">${escape(plan.explanation)}</p><div class="plan"><h3>Saved plan</h3><div class="actions">${plan.actions.map(action => `<span>${escape(action)}</span>`).join('')}</div>${outcome}</div></div></div>`; setStep(plan.state === 'completed' ? 4 : 1); } catch (error) { thread.innerHTML = `<div class="plan">${escape(error.message)}</div>`; } }
