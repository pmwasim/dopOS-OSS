...
class OperationsService:
    ...
    def plan_for_request(self, work_item_id: int) -> dict[str, Any]:
        ...
        if any(word in request for word in ("workspace", "document", "documents", "folder", "folders", "file", "files", "file type", "file types", "extension count", "extension counts", "document types", "supported extensions", "allowed document types", "allowed extensions", "catalog size", "workspace size", "total bytes", "unsupported skipped", "skipped unsupported", "truncated listing", "listing truncated", "listing limit", "result limit")):
    ...
    def workspace_status(self, query: str = "", limit: int = 100) -> dict[str, Any]:
        ...
        if truncated:
            message = f"{message} Document listing truncated at the request limit."
        revision_input = "\n".join(
