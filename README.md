...
Its local API exposes `GET /health` (including audit-chain and execution-safety state), `GET /today` (needs-your-decision, active work, activity, and recovery summary), `GET /autonomous-loop` (bounded, read-only loop-evidence summaries), `GET /workspace` (a metadata-only local document inventory), and `GET /workspace?query=<filename-or-path>` (a bounded metadata-only filename/path search), `GET /diary` (raw technical evidence), `GET /journal` (readable operational entries), `GET /journal.md` (offline Markdown export), `POST /work-items`, `POST /plans`, `POST /plans/{id}/approve`, and `POST /plans/{id}/execute`. Requests are deterministically routed only to explicitly allowlisted local actions. Every action is frozen in a plan and requires approval before execution.
