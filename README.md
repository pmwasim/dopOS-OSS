...
Its local API exposes `GET /health` (including audit-chain and execution-safety state), `GET /today` (needs-your-decision, active work, activity, recovery, and queue summary), `GET /autonomous-loop` (bounded, read-only loop-evidence summaries), `GET /autonomous-loop/queue` (ordered, metadata-only Markdown work-item candidates), `GET /workspace` (a metadata-only local document inventory with a catalog revision), and `GET /workspace?query=<filename-or-path>` (a bounded metadata-only filename/path search), `GET /diary` (raw technical evidence), `GET /journal` (readable operational entries), `GET /journal.md` (offline Markdown export), `POST /work-items`, `POST /plans`, `POST /plans/{id}/approve`, and `POST /plans/{id}/execute`. An approved Workspace snapshot records only the document catalog revision and count in the immutable plan evidence; it does not read or copy document contents. Requests are deterministically routed only to explicitly allowlisted local actions. Every action is frozen in a plan and requires approval before execution.
